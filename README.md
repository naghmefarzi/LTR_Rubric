# Learning to Rank with LLM-Judge Annotations for IR Reranking

This project implements a learning-to-rank (L2R) framework that uses multi-criteria LLM-based judgments (Exactness, Coverage, Topicality, and Contextual Fit) to improve Information Retrieval (IR) system reranking. The approach uses structured prompting to generate relevance scores (0–3 scale) as features for a list-wise L2R model (Rank-LiPS), evaluated on TREC DL19, DL20, and DL23 datasets.

## Key Findings

- **Weak IR systems** (bottom quartile): Multi-criteria features, particularly Contextual Fit, become primary discriminators, often surpassing original IR system scores
- **Mid-performing systems** (25th–75th percentiles): Contextual Fit and Exactness compete with the system's original relevance scores
- **Top-performing systems** (95th–100th percentiles): Original scores dominate
- Results show dataset-dependent effectiveness, with varying importance patterns across DL 2019, 2020, and 2023
...
**Output:**
- Filtered run files: `train/llama3.3-70b/dl19/filtered_dl19/<system_name>/`
...
**Output:**
- Reranked test runs and metrics: `ranklips-results/llama3.3-70b/dl19/<system>/`
...
**Output:**
- `.txt` file of NDCG@20 scores before reranking
- `.txt` file of NDCG@20 scores after reranking
- Per-run evaluation results: `ndcg_scores.txt` in each runfile-specific folder in `ranklips-results/`

---

## Feature Vector Builder

The `build_feature_vectors.py` script processes JSONL.gz files containing query-document pairs with self-ratings and generates feature vectors in RankLib format for learning-to-rank tasks. It supports flexible feature extraction modes and includes detailed debugging logs.

### Purpose

Extracts features from self-ratings in a JSONL.gz file (e.g., from TREC Deep Learning datasets) and saves them in RankLib format: `<label> qid:<query_id> 1:<feature_1> ... # <doc_id>`. Features are derived from self-ratings for prompt classes like nuggets, questions, and direct (e.g., FagB, HELM).
...
- `--no-one-hot`: Skip one-hot encodings in `one_hot` or default modes
...
- To include `exam_pp.data_model` and other dependencies, use `nix develop`
